...
from app.services.auth import log_operation
...
def manual_scan(
    ...
    # 记录操作日志
    log_operation(db, admin_user.id, "manual_scan", "scan", None, None)
    db.commit()
...
def get_recommendation(resume_id: int, query: str = Query(..., description="岗位需求描述"), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """AI 分析岗位匹配度"""
    r = db.query(Resume).filter(Resume.id == resume_id).first()
    if not r:
        from fastapi import HTTPException
...
async def upload_resume(
    ...
    # 记录操作日志
    log_operation(
        db, current_user.id, "upload_resume", "resume", None,
        json.dumps({"uploaded_count": len(uploaded), "failed_count": len(failed)}),
    )
    db.commit()
...
def batch_delete_resumes(
    ...
    # 记录操作日志
    log_operation(
        db, current_user.id, "batch_delete", "resume", None,
        json.dumps({"ids": req.ids, "deleted": count}),
    )
    db.commit()
...
def batch_restore_resumes(
    ...
    # 记录操作日志
    log_operation(
        db, current_user.id, "batch_restore", "resume", None,
        json.dumps({"ids": req.ids, "restored": count}),
    )
    db.commit()
